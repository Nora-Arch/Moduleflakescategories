...
df <- read.csv("C:\\Users\\David\\OneDrive - huji.ac.il\\Desktop\\Archaeology\\Publications\\In_Proguess\\PhD\\Theory and Methods\\Submission\\Moduleflakescategories\\analysis\\data\\raw_data\\Kalavan_2_T1_Edge_Modification.csv", 
...
df_filtered <- df %>% 
  ...
  select(MODULE.OF.RETOUCH, TECHNOLOGICAL.LENGTH, TECHNOLOGICAL.WIDTH, THICKNESS) %>% 
  na.omit() %>%
  rename(
    Module = MODULE.OF.RETOUCH,
...
p_radar_chart <- ggplot() +
  ...
  coord_fixed() +
  facet_wrap(~ Module) +
  labs(title = "Radar Charts for edge modification modules at Kalavan 2 T1",
...
# Save the plot
ggsave("C:\\Users\\David\\OneDrive - huji.ac.il\\Desktop\\Archaeology\\Publications\\In_Proguess\\PhD\\Theory and Methods\\Submission\\Moduleflakescategories\\analysis\\data\\derived_data\\radar_charts_Kalavan_2_T1.png", plot = p, width = 12, height = 8, dpi = 300, bg = 'white')
```
---

This combined Markdown document includes all the code and explanations in one file. You can now use it as an R Markdown document for reproducible reporting.
